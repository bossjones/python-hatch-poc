...
#     # ctx.run("python ./hatchpoc/api/backend_pre_start.py")
#     # ctx.run("python ./hatchpoc/initial_data.py")
#     # ctx.run("python hatchpoc/dev_serve.py")
...
pip install -r requirements-doc.txt
    ...
    click.secho(
        "[install] install editable version of hatchpoc",
        fg=COLOR_SUCCESS,
    )
    ctx.run("pip install -e .")
...
def rsync(ctx, loc="local", verbose=0, cleanup=False):
    """
    rsync over files to ~vagrant/hatchpoc folder
    ...
    _cmd = r"""
cd && rsync -r --exclude hatchpoc_venv --exclude .vagrant --exclude .git --exclude .env /srv/vagrant_repos/hatchpoc/ ~/hatchpoc/ && sudo chown vagrant:vagrant -R ~vagrant && cd ~/hatchpoc && ls -lta
    """
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python hatchpoc/web.py' | awk '{print $2}' | head -1
    """
